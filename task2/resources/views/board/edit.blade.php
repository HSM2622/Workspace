...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
      ...
      <form action="/qna/{{ $qna->id }}" method="POST" class="flex w-[90%] h-3/4 bg-gray-200  border-x-[1px] border-gray-300 flex-col">
      {{ csrf_field() }}  
      <div class="flex w-full h-14 border-y-2 border-black justify-center items-center text-[18px] font-[600]">글 쓰기</div>
        <div class="flex w-full h-12 border-b-2 border-black justify-between items-center bg-gray-100 text-[18px] font-[600]">
          <div class="flex w-1/2 h-full justify-center items-center">
            제목<input name="title" class="flex w-[70%] h-full ml-4 p-2 text-[15px] border-[1px] border-gray-400 rounded" value="{{ $qna -> title }}"></input>
          </div>
          <div class="flex w-1/2 h-full justify-center items-center">
            카테고리
          </div>
        ...
        <div class="flex w-full h-[75%] bg-white justify-center items-center p-2 text-[20px] font-[600] flex-col border-b-2 border-black">
          ...
          <textarea name="body" class="flex w-[88%] h-[70%] border-[1px] border-gray-400 mt-2 resize-none p-3 focus:outline-none">{{$qna->body}}</textarea>
        ...
        <div class="flex w-full h-14 justify-center items-center border-b-2 border-black">
          ...
          <button type="submit" class="flex w-32 h-10 bg-gray-400 justify-center items-center text-white rounded-full text-[18px]">
            목록으로
          </button>
        </div>
      </div>
